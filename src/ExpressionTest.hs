...
instance Arbitrary TExpr where
  ...
  shrink (TExpr (BinExpr Div e1 e2)) = TExpr <$> [e1, e2] ++ [BinExpr Div e1' e2' | (TExpr e1', TExpr e2') <- shrink (TExpr e1, TExpr e2), eval e2' /= Just 0]
  shrink (TExpr (BinExpr Mod e1 e2)) = TExpr <$> [e1, e2] ++ [BinExpr Mod e1' e2' | (TExpr e1', TExpr e2') <- shrink (TExpr e1, TExpr e2), eval e2' /= Just 0]
